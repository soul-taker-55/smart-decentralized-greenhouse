...
**Telemetry interval — OPEN.** The mock currently publishes every 10 s and the contract does
not fix a value. Greenhouse conditions do not change meaningfully at that rate, so most of what
is stored is sensor noise around an unchanged value. Moving to 30 s or 60 s cuts storage volume
by 3–6× and produces charts that show trends rather than jitter. The edge control loop is
unaffected — only the publish rate changes. **Decide and record here before 04d is built**,
because changing it afterwards means a dataset with two different sampling rates in it.
