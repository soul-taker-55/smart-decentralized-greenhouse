...
**Supersedes:** the Parts 1–2 draft of the same name
...
> | Implementation | Role |
> |---|---|
> | ESP32 firmware (Phase 2–3) | publishes telemetry, consumes config, enforces the safety envelope |
> | MQTT→DB bridge (Phase 4) | consumes everything, writes to Postgres |
> | REST backend (Phase 5a) | publishes config and commands, computes and verifies the canonical hash |
> | Browser (Phase 6) | computes the canonical hash and signs it |
...
| `down/config` | 1 | **yes** | The reconnect design depends on this. Verified in Step 0a, including across a broker restart. |
...
**Phase 3 firmware must call `setBufferSize(1024)`.** Required, not advice.
...
the server having to detect the reconnection and re-push. Verified in Step 0a, including across
...
policy deterministically. That is what makes Phase 7 a real comparison later: fuzzy inference
...
**Pin assignments.** Fixed in Phase 1 and locked at wiring doc v1.7.
...
not autonomous — it is running on cached permission. And Zero Trust, one of the four stated
pillars, means precisely that trust is not granted by network position; an edge accepting
whatever arrives from the broker is perimeter security, the model Zero Trust exists to replace.

### Statement for the thesis

> The edge does not verify signatures or hashes. Cryptographic verification occurs server-side,
> where the keys and the approval quorum live. The edge enforces an independent safety envelope
> derived from equipment limits, which rejects unsafe configurations regardless of provenance.
> These are complementary gates: the server controls *who may change things*; the firmware
> controls *what values are physically permissible*.
...
| Rule | Specification |
|---|---|
| Key order | Lexicographic by UTF-8 code point, **applied recursively** to nested objects |
| Whitespace | None. No spaces after `:` or `,`, no newlines, no trailing newline |
| Encoding | UTF-8, no BOM |
| Numbers | **Integers only.** No decimal point, no exponent, no leading zeros, no `+`. Negative numbers use a single leading `-`. |
...
> **Action item:** compute SHA-256 over the canonical string above and record the hex digest
> here as the frozen expected value. Do this once, in one implementation, then check the others
> against it. A second vector using the real `cfg` shape should be added once the engineer
...
*Parts 1–4 complete. Frozen for Step 0c (mock simulator) and Phase 4 (bridge and schema).*
