...
export async function getServerEstop(ghId = config.ghId) {
  ...
  if (r.rows.length === 0) {
    return { seq: 0, state: 'clear', by: null, reason: null, at: null, everUsed: false };
  }
...
export async function setEstop({ state, reason, actor, publisher }) {
  ...
  const record = await transaction(async (client) => {
    const inserted = await client.query(
      ...
      [
        ...
        actor.id,
        actor.role,
        JSON.stringify({ seq, state, reason: reason ?? null, sessionAttributed: true }),
      ]
    );
  ...
  try {
    await publisher.publishEstop({ seq, state, reason: reason ?? null, by: actor });
    published = true;
  } catch (err) {
